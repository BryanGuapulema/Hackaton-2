...
def _ctas_orders_to_silver(year: int, month: int, month_z: str):
    ...
    WITH stage AS (
      SELECT
        ...
        CAST(date_parse(OrderDate, '%m/%d/%Y') AS DATE)  AS OrderDate,  -- M/d/yyyy
    ...
    DROP TABLE IF EXISTS {DB}.tmp_orders_valid_{year}_{int(month_z)};
    DROP TABLE IF EXISTS {DB}.tmp_orders_invalid_{year}_{int(month_z)};
    """
    run_athena(sql)
